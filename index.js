...
const httpServer = createServer(async (req, res) => {
  ...
  if (req.url === "/health") {
    ...
    res.writeHead(200);
    // 실제 환경변수 목록에서 NAVER 관련 키를 모두 출력
    const naverKeys = Object.keys(process.env).filter(k => k.includes("NAVER"));
    res.end(JSON.stringify({
      status: "ok",
      version: "4.0.0",
      naver: !!NAVER_CLIENT_ID,
      naverKeys,   // 어떤 이름으로 들어왔는지 확인
      clientIdLen: process.env.NAVER_CLIENT_ID?.length || 0,
      secretLen:   process.env.NAVER_CLIENT_SECRET?.length || 0,
    }));
    return;
  }
